...
if do["3"]:
    ...
    def calculations(generator_power: float,
        ...
        for I in I_1:
            ...
            i_2 = I-I_T
            V_B.append(power/I*(1+0.2j))
            I_t.append(I_T)
            I_2.append(i_2)
            if power_test:
                print("Supplied power by VSC-B minus power drawn from all components: ",
                      power*(1+0.2j)-I**2*impedance_primary-I_T**2*imp_core-i_2**2*impedance_sec_ref_prim-(i_2-I_poc_rp)**2*imp_cap-I_poc_rp*V_grid_ref_prim)
